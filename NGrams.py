...
def preProcess(line): #strip line,
  break into words# extracting the text sentence from each line
line = "".join(line.split())
line = line.lower()# to lower
case
...
def countNgram(tokens, order):
  pairs = dict()
  if len(tokens) < order: #print pairs
  return pairs
  ...
  triGramPairsEn = combinedCounts(trigramEn)# smoothen# smoothen(unigramPairsEn, 0)# smoothen(bigramPairsEn, 1)
  smoothen(triGramPairsEn, 2)# finalizeCount(unigramPairsEn)# finalizeCount(bigramPairsEn)
  finalizeCount(triGramPairsEn)# ngramPrint(unigramPairsEn)# ngramPrint(bigramPairsEn)
